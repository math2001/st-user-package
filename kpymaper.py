...
set_option('match_all_default_value', True)

NORMAL_MODE = get_context('six_is_in_insert_mode', False)

class contexts:
    word_before = get_context('preceding_text', 'regex_contains', '[\\w\']$')
    iswidget = get_context('setting.is_widget')
    javascript = get_context('selector', 'source.js')
    python = get_context('selector', 'source.python')

LEADER = ','
# vim (six)
with context('setting.is_widget', False):
    add('j', 'k', 'six_press_key', {'key': '<Esc>'}, get_context('six_is_in_insert_mode'))
    add(';', 'six_press_key', {'key': ':'}, get_context('six_is_in_insert_mode', False))
    add(LEADER, 'b', 'build', NORMAL_MODE)
...
# add('alt+d', 'insert_date', {'format': '%A %d %B %Y @ %H:%M'})
